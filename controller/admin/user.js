...
class UserController extends RestController {
  constructor() {
    ...
    Role.findOne({ where: { name: this.defaultUserRole } }).then((result) => {
      if (!result) {
  ...
  index(req, res) {
    ...
    const Role = this.models['Role'];
    data.include = [{ model: Role, as: 'roles' }];
    data.distinct = true;
  ...
  create(req, res) {
    ...
    const result = pw.hash(value.password).then((hash) => {
      value.password = hash;
      delete value.roles;
      return Role.findAll({ where: { name: { $in: creationRoles } } });
    }).then((roles) => {
  ...
  update(req, res) {
    ...
    const result = Promise.resolve().then(() => {
      ...
      if (value.roles) {
        return Role.findAll({ where: { name: { $in: value.roles } } }).then((roles) => {
          updateRoles = roles;
        });
      }
    }).then(() => {
