...
export default {
  ...
  created() {
    this.changePage(1);
    this.$http.get('permissions?limit=1000').then((res) => {
  ...
  methods: {
    changePage(page) {
      ...
      this.$http.get(`roles?limit=10&offset=${((page || 1) - 1) * this.pageSize}`).then((res) => {
        const data = res.data.data;
    ...
    editPermission(result) {
      if (result) {
        ...
        this.$http.put(`roles/${this.dataModel.id}/permissions`, {
    ...
    destroy(result) {
      if (result && this.destroyData) {
        this.$http.delete(`roles/${this.destroyData.id}`).then(() => {
