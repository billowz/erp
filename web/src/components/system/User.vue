...
export default {
  ...
  created() {
    this.changePage(1);
    this.$http.get('roles?limit=100').then((res) => {
  ...
  methods: {
    changePage(page) {
      ...
      this.$http.get(`users?limit=10&offset=${((page || 1) - 1) * this.pageSize}`).then((res) => {
        const data = res.data.data;
    ...
    destroy(result) {
      if (result && this.destroyData) {
        this.$http.delete(`users/${this.destroyData.id}`).then(() => {
